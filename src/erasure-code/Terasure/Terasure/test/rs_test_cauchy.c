...
void test_decode_correctness(rs_conf conf, uint8_t **data, uint8_t **encode_data,
	...
	for(int test_id = 0; test_id < test_round; test_id++){
		...
		// printf("\n");
		// for(int i = 0; i < n; i++){
		// 	for(int j = 0; j < k; j++) printf("%d ", encode_data[i][j]);
		// 	printf("\n");
		// }
		// printf("********\n");
		// for(int i = 0; i < k; i++){
...
int main(int argc, char **argv) {
	...
	cauchy_matrix(&conf);
	test_decode_correctness(conf, data, encode_data, memory_pre_allocated, decode_data);
	rs_free_conf(&conf);

	printf("/*******Regenerate Correctness********/\n");
	cauchy_matrix(&conf);
	test_regenerate_correctness(conf, data, encode_data, memory_pre_allocated, decode_data);
	rs_free_conf(&conf);
