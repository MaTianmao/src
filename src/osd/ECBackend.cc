  ...
  for(auto i = op.to_read.begin();
    ...
    for (auto j = i->second.begin(); j != i->second.end(); ++j) {
      ...
      if(op.subchunks.find(i->first)->second.size() == 1 &&
        op.subchunks.find(i->first)->second[0].second == 1){
        r = store->read(ch,
        ...
        for(int h = 0; h < (int)j->get<1>(); h += sinfo.get_chunk_size()){
          for(auto &&k : op.subchunks.find(i->first)->second){
            bufferlist bj;
            r = store->read(ch,
                        ghobject_t(i->first, ghobject_t::NO_GEN, shard),
                        j->get<0>() + h + k.first * subchunk_size,
  ...
  const ZTracer::Trace &trace)
{
  ...
  if (rop.do_redundant_reads || (!rop.for_recovery && rop.in_progress.empty())) {
    for (map<hobject_t, read_result_t>::const_iterator iter =
      ...
      int err;
      if ((err = ec_impl->sub_read_to_decode(want_to_read, have, &dummy_minimum)) < 0) {
	dout(20) << __func__ << " sub_read_to_decode failed" << dendl;
  ...
  map<pg_shard_t,vector<pair<int,int> > > *to_read)
{
  ...
  if(for_recovery){
    ...
    int r = ec_impl->sub_read_to_decode(want, have, &need);
    if(r < 0)
      return r;
  }
  ...
  if (do_redundant_reads) {
    ...
    default_subchunks.push_back(make_pair(0, 1));
    for(auto &&i: have){
      need[i] = default_subchunks;
    }
  } 
  ...
  map<pg_shard_t, vector<pair<int, int> > > *to_read)
{
  ...
  for (set<int>::iterator i = need.begin();
    ...
    if (avail.find(*i) == avail.end())
      to_read->insert(make_pair(shards[shard_id_t(*i)], default_subchunks));
  }
  return 0;
}
...
void ECBackend::do_read_op(ReadOp &op)
{
  ...
  for (map<hobject_t, read_request_t>::iterator i = op.to_read.begin();
    ...
    for (map<pg_shard_t, vector<pair<int, int> > >::const_iterator j = i->second.need.begin();
	 j != i->second.need.end();
	 ++j) {
      if (need_attrs) {
	...
	sinfo.aligned_offset_len_to_chunk(make_pair(j->get<0>(), j->get<1>()));
      for (map<pg_shard_t, vector<pair<int, int> > >::const_iterator k = i->second.need.begin();
	   k != i->second.need.end();
	   ++k) {
